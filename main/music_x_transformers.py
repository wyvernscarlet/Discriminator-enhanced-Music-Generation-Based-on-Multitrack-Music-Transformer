...
class Discriminator(nn.Module):
    def __init__(self, input_dim, num_heads=8, ff_dim=32, num_transformer_blocks=1, mlp_units=256, dropout=0.2,
                 ignore_index=-100, pad_value=0):
        super(Discriminator, self).__init__()
        self.transformer_blocks = nn.Sequential(
            *[TransformerBlock(input_dim=input_dim, num_heads=num_heads, ff_dim=ff_dim, dropout=dropout) for _ in range(num_transformer_blocks)]
        )
        self.global_avg_pooling = nn.AdaptiveAvgPool1d(1) #check this row
